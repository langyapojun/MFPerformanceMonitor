...
@property (nonatomic, strong) NSMutableArray<MFPerformanceInfo *> *controllerPerformanceInfo;
...
- (void)initViews
{
    ...
    [lineChart setXLabels:[_controllerPerformanceInfo valueForKeyPath:@"intervalSeconds"]];
    ...
    memChartData.getData = ^(NSUInteger index){
        CGFloat memoryUsage = weak_self.controllerPerformanceInfo[index].memoryUsage;
    ...
    cpuChartData.getData = ^(NSUInteger index){
        CGFloat cpuUsage = weak_self.controllerPerformanceInfo[index].cpuUsage;
